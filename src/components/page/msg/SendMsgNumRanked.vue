        ...
        <el-table :data="records" highlight-current-row v-loading="listLoading"
            ...
            <el-table-column
                ...
                <template scope="scope">
                    <router-link
                        :to="{ name: '用户聊天消息数量趋势',params: {userIds : scope.row._id, nickname: scope.row.nickname, previousPage: '/sendMsgNumRanked'}}">
                        {{ scope.row.sendNum }}
...
<script>
    ...
    import sendMsgNumsRanked from 'static/requestList/sendMsgNumsRanked.json';

    export default {
        data() {
            return {
                title: '用户发送消息数 TOP 50',
                pickerOptions1: {
                    shortcuts: [{
                        text: '今天',
                        onClick(picker) {
                ...
                aggregate(params)
                    .then(data => {
                        ...
                        for (let i = 0; i < data.length; i++) {
                            this.userIds.push(data[i]._id.fromUserId);
//                            this.friNum.push(data[i].num);

                            this.records.push({
                                '_id': data[i]._id.fromUserId,
                                'nickname': '',
                                'userId': '',
                                'seqid': '',
                                'sendNum': data[i].num,
